...
%CONTAINER_RUNTIME% compose --env-file env/.env.local --profile local down -v

echo Building images...
%CONTAINER_RUNTIME% compose --env-file env/.env.local --profile local build
...
if %ERRORLEVEL% EQU 0 (
    ...
    if %ERRORLEVEL% EQU 0 (
        ...
        echo   Password: admin
        echo.
        echo To view logs: %CONTAINER_RUNTIME% compose --env-file env/.env.local --profile local logs -f
        echo To stop:     %CONTAINER_RUNTIME% compose --env-file env/.env.local --profile local down
        echo.
    ) else (
        echo.
