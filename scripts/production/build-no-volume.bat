...
chcp 65001 >nul
echo ======================================
echo Apache Superset 本番環境 完全再構築
...
powershell -Command "podman compose --env-file ..\..\env\.env.production down -v"
echo.

echo イメージをキャッシュなしで再ビルドしています...
powershell -Command "podman compose --env-file ..\..\env\.env.production build --no-cache"
